...
def getUserInfo(server, userid):
    ...
    if server.shortname not in serverCaches:
...
def getEventUserId(event):
    if 'userid' in event.parms: return event.parms.userid
    if 'srcuserid' in event.parms: return event.parms.srcuserid
    if 'kickerid' in event.parms: return event.parms.kickerid
    return None
...
def prittifyName(userid, userinfo):
    ...
    userName = userinfo.get('userName') or userinfo.get('username') or ""
    if nickname: name = f"\"{nickname}\""
    if userName:
        if name and name != userName: name += f" ({userName})"
...
def prittifyEvent(server, event):
    ...
    if userid is not None and userid != '0':
        ...
        userTypeString = 'admin' if userinfo.get('admin') == True else 'user'
    match event.event:
        case 'loggedin':
...
class Trigger(TriggerBase):
    def __init__(self, *args, **kwargs):
        ...
        if 'srcuserid' in self.event.parms and self.server.me.userid == self.event.parms.srcuserid: return
    ...
    def playSound(self):
        ...
        if self.event.event == 'messagedeliver':
            messageType = str(self.event.parms.type)
            if messageType == '1': sound = 'user'
            elif messageType == '2': sound = 'channel'
            elif messageType == '3': sound = 'broadcast'
            elif messageType == '4': sound = 'message'
            else: return
        else:
    ...
    def handleCache(self):
        ...
        if self.event.event == 'updateuser':
            ...
            if 'users' not in serverCaches[self.server.shortname]: return
            if self.event.parms.userid not in serverCaches[self.server.shortname]['users']: return
    ...
    def initializeCache(self):
        ...
        for u in self.server.users:
            ...
            userInfo = {
                ...
                'nickname': u.get('nickname', f"user{u['userid']}"),
                'admin': True if u.get('usertype') == '2' else False
            }
            serverCaches[self.server.shortname]['users'][u['userid']] = userInfo
